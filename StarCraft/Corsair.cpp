...
HRESULT CCorsair::Initialize(void)
{
	...
	lstrcpy(m_tInfo.szObjKey, L"Corsair");
	lstrcpy(m_tInfo.szStateKey, L"Normal");
	lstrcpy(m_tUnitInfo.szName, L"Corsair");
	...
	m_tAttDelay = FRAME(0.f, 10.f, 5.f);
	m_eRenderType = RENDER_AIR_UNIT;
	m_iView = 9;
	m_tUnitInfo.iMp = 50;
	m_tUnitInfo.iMpMax = 200;
...
void CCorsair::Render(void)
{
	D3DXMATRIX matScale, matTrans;

	if (m_bSelect)
	{
		CObjectRenderMgr::SelectUnitUIRender(this, 2, 7.f, 3, 25.f);
	
		CGameObject* pInterpace = ObjMgr->GetList()[OBJ_INTERPACE].front();

		if (m_eTeamType == pInterpace->GetTeamType())
		{
			///Mp Frame
	...
	const TEX_INFO* pShuttletexture = NULL;
	if (m_dwState != UNIT_ATTACK)
	{
		pShuttletexture = TextureMgr->GetTexture(m_tInfo.szObjKey
...
void CCorsair::SelectKeyCheck(void)
{
...
void CCorsair::CheckOneClick(void)
{
	if (m_dwState == UNIT_DEAD || m_dwUnitState == UNIT_STASISFIELD)
		return;
	...
	if (m_dwState == UNIT_ATTACK)
	{
		if (m_tAttDelay.fFrame > m_tAttDelay.fMax)
		{
			...
			if (m_tFrame.fFrame + m_iFrameCount * 17 > m_tFrame.fMax)
			{
				...
				if (m_AttTarget != NULL)
				{
					SoundMgr->PlaySound(L"pcorlasr1.wav", CHANNEL_EFFECT_ATT, 0.5f);
					ObjMgr->AddGameObject(CFactory<CAttEffect>::CreateGameUnitEffectTarget(m_AttTarget->GetPos(), ERRECT_CORSAIR_ATT, m_AttTarget, m_tUnitInfo, m_eTeamType), OBJ_EFFECT);
				}
			}
		}
	}
