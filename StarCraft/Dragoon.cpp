...
HRESULT CDragoon::Initialize(void)
{
	...
	lstrcpy(m_tInfo.szObjKey, L"Dragoon");
...
int CDragoon::Update(void)
{
	...
	if (D_CAST(CInterpace, pInterpace)->GetUpgradeCount(m_eTeamType, UPGRADE_DRAGOON_RANGE) == 1)
		m_fRange = 90.f;
	else
		m_fRange = 0.f;
	...
	if (m_bDeadCheck)
	{
		...
		ObjMgr->AddGameObject(CFactory<CEffect>::CreateGameEffect(m_tInfo.vPos, EFFECT_DRAGOON_DEAD), OBJ_EFFECT);
		return 1;
	}
	Clooking();
	SelectKeyCheck();
...
void CDragoon::Render(void)
{
	...
	if (m_bSelect)
	{
		CObjectRenderMgr::SelectUnitUIRender(this, 2, 7.f, 3, 30.f);
	}
...
void CDragoon::SelectKeyCheck(void)
{
...
void CDragoon::CheckOneClick(void)
{
	if (m_dwUnitState == UNIT_STASISFIELD)
		return;
	if (m_dwState == UNIT_DEAD)
...
void CDragoon::Funtion(void)
{
	...
	if (m_bUnitAtt && m_AttTarget != NULL &&
		fabs(m_AttTarget->GetPos().x - m_tInfo.vPos.x)  < fabs(m_AttTarget->GetInfo().vSize.x / 2.f - m_tInfo.vSize.x / 2.f) + 100.f + m_fRange &&
		fabs(m_AttTarget->GetPos().y - m_tInfo.vPos.y)  < fabs(m_AttTarget->GetInfo().vSize.y / 2.f - m_tInfo.vSize.y / 2.f) + 100.f + m_fRange)
	{
		m_pAStar->AStarClear();
...
void CDragoon::Frame(void)
{
	...
	case UNIT_STAND:
	case UNIT_HOLD:
		m_tFrame = FRAME(0.f, 6.f, 7.f);
		break;
	case UNIT_ATTACK:
		m_tFrame = FRAME(0.f, 20.f, 7.f);
		break;
	case UNIT_DEAD:
		m_tFrame = FRAME(0.f, 5.f, 6.f);
		break;
	}
	...
	if (m_dwState == UNIT_MOVE)
	{
		...
		(m_dwState == UNIT_STAND || m_dwState == UNIT_HOLD)
	{
		lstrcpy(m_tInfo.szStateKey, L"Normal");
	}
		...
		if (m_tAttDelay.fFrame > m_tAttDelay.fMax)
		{
			...
			if (m_tFrame.fFrame + m_iFrameCount * 17 > m_tFrame.fMax)
			{
				...
				if (m_AttTarget != NULL)
				{
					...
					ObjMgr->AddGameObject(CFactory<CAttEffect>::CreateGameUnitEffectTarget(m_tInfo.vPos, EFFECT_DRAGOON_ATT, m_AttTarget, m_tUnitInfo), OBJ_EFFECT);
